...
const VanavihariConsolidationReport = () => {
  ...
  const filteredCreditedData = useMemo(() => {
    if (!fromDate && !toDate) return creditedData;

    return creditedData.filter((item) => {
  ...
  }, [creditedData, fromDate, toDate]);

  const filteredDebitedData = useMemo(() => {
    if (!fromDate && !toDate) return debitedData;

    return debitedData.filter((item) => {
      const itemDate = new Date(item.date);
      const from = fromDate ? new Date(fromDate) : null;
      const to = toDate ? new Date(toDate) : null;
  ...
  const creditedColumns = [
    ...
    { title: "<strong>Days</strong>", data: "days" },
    {
      ...
      render: (data: number) =>
        data == null ? "" : `₹${data.toLocaleString("en-IN")}`,
    },
  ...
  return (
    <div className="w-full max-w-full overflow-hidden p-6">
      ...
      <div className="flex flex-col md:flex-row justify-end gap-4 mb-6 items-end">
        {/* From Date */}
