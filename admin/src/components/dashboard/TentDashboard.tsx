...
import { CalendarDays, Tent, Users, LogOut, CreditCard, TrendingUp } from "lucide-react";
import {
...
export default function TentDashboard() {
  const apiUrl = (import.meta.env.VITE_API_URL as string) || 'http://localhost:5000';
  const [selectedTentSpot, setSelectedTentSpot] = useState<string>('all');
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        if (result.success) {
          ...
          console.error('Failed to fetch tent dashboard data:', result.error);
        }
      } catch (error) {
        console.error('Error fetching tent dashboard data:', error);
      } finally {
        setIsLoading(false);
      }
    };
    ...
    return (
      <div className="p-4 md:p-6 flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Loading tent dashboard...</p>
        </div>
      </div>
    );
  }

  const { stats, paymentBreakdown, last5Bookings, occupancy7Day, tentSpots } = dashboardData;
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold">Tent Dashboard</h1>
          <p className="text-sm text-muted-foreground">Overview of tent spot operations</p>
        </div>
        <Select value={selectedTentSpot} onValueChange={setSelectedTentSpot}>
          <SelectTrigger className="w-full sm:w-[220px]">
            <SelectValue placeholder="Select tent spot" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">All Tent Spots</SelectItem>
            {tentSpots.map(ts => (
