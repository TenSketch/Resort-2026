...
const AddRoomForm = () => {
    ...
    const fetchNextRoomId = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${apiBase}/api/rooms/next-room-id/${formData.resortId}`,
          { headers },
        );
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      setFormData((prev) => ({
        ...prev,
        roomImages: [...prev.roomImages, ...fileArray],
      }));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.cottageTypeId)
      form.append("cottageType", formData.cottageTypeId);
    ...
    if (formData.weekDaysRate)
      form.append("price", String(formData.weekDaysRate));

    if (formData.weekEndRate)
      form.append("weekEndRate", String(formData.weekEndRate));
    if (formData.noOfGuests)
      form.append("noOfGuests", String(formData.noOfGuests));
    if (formData.extraGuests)
      form.append("extraGuests", String(formData.extraGuests));
    if (formData.noOfChildren)
      form.append("noOfChildren", String(formData.noOfChildren));
    if (formData.chargesPerBedWeekDays)
      form.append(
        "chargesPerBedWeekDays",
        String(formData.chargesPerBedWeekDays),
      );
    if (formData.chargesPerBedWeekEnd)
      form.append(
        "chargesPerBedWeekEnd",
        String(formData.chargesPerBedWeekEnd),
      );
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            <div className="space-y-2">
              ...
              >
                <option value="">
                  {resortsLoading ? "Loading..." : "-- Select Resort --"}
                </option>
                {resorts.map((resort) => (
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-slate-500">
                Auto-generated based on resort. You can edit if needed.
              </p>
            </div>
            <div className="space-y-2">
              <Label
            ...
            <div className="space-y-2 md:col-span-2 lg:col-span-3 xl:col-span-4">
              ...
              >
                ...
                {formData.roomImages.length > 0 &&
                  `(${formData.roomImages.length} selected)`}
              </Label>
