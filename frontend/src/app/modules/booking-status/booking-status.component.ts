...
export class BookingStatusComponent {
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe((params) => {
      ...
      // Check if this is a tent booking
      ...
      .subscribe({
        next: (response) => {
          ...
          if (response.success && response.bookings) {
            ...
            this.processRoomBooking(booking);
          } else {
            this.bookingStatus = 'failed';
            setTimeout(() => {
  ...
  fetchTentBookingDetails(): void {
    // Fetch tent reservation details
    this.http
      .get<any>(`${this.api_url}/api/tent-reservations/booking/${this.bookingId}`)
      .subscribe({
        next: (response) => {
          this.showLoader = false;
  ...
  processRoomBooking(booking: any): void {
    ...
    if (booking.paymentStatus === 'paid' && booking.status === 'reserved') {
      ...
      this.reservationDetails = {
        guestName: booking.fullName || booking.user?.name,
  ...
  processTentBooking(booking: any): void {
    ...
    if (booking.paymentStatus === 'paid' && booking.status === 'reserved') {
      ...
      const tentsArray = booking.tents?.map((tent: any) => ({
        room_name: tent.tentId || tent.name || 'Tent',
        cottage_type: tent.tentType?.tentType || 'Camping Tent'
      })) || [{ room_name: 'N/A', cottage_type: 'N/A' }];
      ...
      this.reservationDetails = {
        ...
        resortName: tentSpotName,
        transactionId: transactionId,
        resortLocation: booking.tentSpot?.location || tentSpotName,
        bookingId: booking.bookingId,
